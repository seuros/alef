...
1. **Version sync** — always use `task set-version -- X.Y.Z` to bump the version. This rewrites `Cargo.toml`, `alef.toml`, and `src/core/template_versions.rs::ALEF_REV`, regenerates `schemas/alef.schema.json` (via `cargo run -- schema --schema-version`), and runs `cargo update` — all in one shot. The schema diff is expected output, not drift; never hand-edit it.
...
6. **GitHub release** — `gh release create vX.Y.Z --title "vX.Y.Z" --notes-file <extracted CHANGELOG section>`. **This is the step that publishes.** `Publish` triggers on `release: types: [published]` only — a tag push on its own runs nothing. v0.55.2 and v0.55.3 were tagged and pushed without a release and never reached crates.io.
