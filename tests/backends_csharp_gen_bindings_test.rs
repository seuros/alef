...
/// `ALEF_FFI_CONVERSION_ERROR`, never a user taxonomy code (`validate_error_taxonomy`
/// rejects any explicit `error_code` below 100), so a user error variant literally
/// named `InvalidInput` must never be constructed off `code == 1`. It dispatches
/// through the same message-prefix mechanism as every other variant instead. ~keep
#[test]
fn test_invalid_input_variant_does_not_hijack_ffi_conversion_error_code() {
    ...
    let api = ApiSurface {
        ...
        errors: vec![ErrorDef {
            ...
            rust_path: "test::RequestError".to_string(),
            original_rust_path: String::new(),
            variants: vec![ErrorVariant {
                name: "InvalidInput".to_string(),
                error_code: None,
