...
/// the real extension's `#[php(constructor)]` cannot accept (e.g. `Optional(Json)`, which has
...
fn json_field_is_excluded_from_constructor_but_reachable_via_getter() {
    ...
    // --- positive: `value` (Optional(Json)) is not representable as a constructor param ---
    assert!(
        !content.contains("$value"),
        "Optional(Json) field `value` has no ext-php-rs constructor-param support and must not \
