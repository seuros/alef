...
Alef generates binding code and build scripts that downstream projects compile and run. The security-relevant surface is **code generation**: any input (a schema, a name, a doc comment) that can escape its context in generated source, inject a build-script command, or emit an unsafe FFI boundary is in scope.
