...
- Advance the Python accessor's owner-type cursor through a map key access to the map's VALUE
  ...
  on an unsubscriptable object. The map's value type is now recorded as a private resolver
  traversal edge, so the classification is derived rather than inherited; a map whose values
  name no IR type still keeps the previous owner, since nothing was derived.
...
- Select the Node/TypeScript `not_error` assertion's sync-vs-async wrapper from the call's real
...
- Generated Python converters now type-check mixed optional constructor defaults with Pyrefly.
...
- Decode generated Dart typed-array elements without assuming their JSON values
  are objects, preserving scalar and untagged enum representations.
...
- fix(build,python): resolve the Python build through the configured `python_package_manager` and its locked environment, and activate the `extension-module` feature the generated manifest declares. Three sites restated both facts independently -- the default build command, the build-command builder, and `alef publish` -- so a consumer whose `PATH` maturin predates their pinned requirement built against the wrong toolchain, and every wheel shipped without `abi3`. The readiness precondition now targets the tool that will actually run the build rather than bare `maturin`, which would otherwise skip the build on exactly the machines the fix targets. With `[crates.output] python` unset the command also emitted `--manifest-path /Cargo.toml`, the dangling-path bug already fixed in the napi and wasm-pack arms; left alone it would have made the feature probe read a nonexistent file and pass green.
- fix(snippets): refuse a validation level whose build artifacts do not exist instead of spawning a toolchain process per snippet against them. `alef all` never builds the full per-language artifacts that the `typecheck`/`compile`/`run` levels need -- only `alef build` does -- so a consumer with `docs.snippets.validation_level = "compile"` reached that stage on the default path with the artifacts absent and paid one real process per snippet that could not produce a true positive, only wall clock. The artifacts are now probed once per session, before anything is spawned, and a single `WARN` names the language, the level, the snippet count, the missing artifacts, and the remedy. The per-snippet fallback also stopped rediscovering one shared missing artifact once per snippet that depends on it.
- fix(snippets): report snippets skipped by that preflight rather than absorbing them. They carry `SnippetStatus::Unavailable` with `unresolved_dependency` -- bit for bit the classification the per-snippet path already produced on discovering the same missing artifact the expensive way -- plus a `preflight_skipped` count printed as its own line. They stay out of `fully_verified`, still trip `checked_nothing()`, and still pass or fail `--strict` exactly where they did before: detecting the fact earlier changes what a run costs, never what it concludes.
- fix(snippets): count a validation timeout separately from a validation failure. A timeout still lands in `Errors` and still fails the run -- an unbounded toolchain is a real problem -- but is now named in the summary and the strict-bail message, so a reported "411 failed" can be read as what it is. A batch timeout, which charges one expired stopwatch to every snippet in the group, is the case this matters most for.
- fix(generate): stop `alef generate`/`alef all` warning that a Go, Java or C# native library could not be staged and advising `alef build --release`. Those commands never build the `-ffi` cdylib, so on any unbuilt checkout the warning fired once per FFI-dependent language for a condition the invoked command never intended to satisfy. `StagingProfile` gained a third variant because one value was answering two questions -- *which* artifact to stage, and whether a miss is news; `alef test --e2e`, which is about to link that library, keeps the warning and the build hint.
- fix(verify): distinguish a frozen file whose withheld content has DRIFTED from one alef would have written byte-for-byte. Both the write-time refusal tally and `alef verify`'s frozen report treated every withheld write identically, so a consumer could not tell a file that had genuinely fallen behind from one that was already current and merely unmarked -- the ordinary state of a tree predating alef stamping an extension. The comparison is one predicate shared by both, and it asks `ensure_generated_header` itself rather than stripping a provenance prefix: alef's header is a three-line block inserted *below* a shebang or `<?php`, so a prefix strip reported every body-identical generated shell script as stale on every run.
- fix(verify): report markable-but-unmarked files under a rewritten e2e root, which fell through every existing set. `test_apps/php/install.sh` is `generated_header: false` with no marker, so the managed walk skipped it; `.sh` is markable, so the unmarkable walk skipped it too. It appeared in no finding and no count, while the e2e writers' `overwrite = true` meant the guard refused that differing write on every run -- which is how three consumer repos shipped PHP installers one to two releases stale. The verify coverage block also no longer calls those files "the documented user-owned steady state, not drift" over files it had compared none of.
- fix(e2e,wasm): import the enum classes the generated test body references. Two wasm emission sites wrote `{var}.{field} = {EnumType}.{Member}` without recording the enum, while four sibling sites did, and the recorded set was consumed only under a `lang == "node"` guard -- so the wasm import list never looked at it at all. The import source was hand-written `enum_fields` config, which no IR-typed enum field ever appears in, giving a reference with no import. Emission now records as it emits, so the import set and the body share one derivation rather than two.
- fix(e2e,wasm): import the *prefixed* class name in the handle-config path. The membership guard was `String::contains`, a substring test that let `WasmExtractInputKind` satisfy a check for `ExtractInputKind` -- which is why the existing nested-types test passed over the drift instead of catching it. The guard now compares whole tokens.
- fix(hash): stamp an INI-syntax file with its own comment character. `.npmrc` carried a `; alef:...` prose marker and a `// alef:hash:...` stamp directly beneath it -- two comment syntaxes in one file, the second of which npm does not parse as a comment. "Which comment character does this format use" was derived twice: `render_header` had a `Semicolon` style, while the injectors and parsers each re-sniffed the marker line against their own hardcoded list with no `;` arm, so INI fell to the `//` default. One table now serves emission and read-back, so a shape alef can write is by construction one it can read. Existing `//` stamps stay recognised and migrate on the next generate with an unchanged hash value.
- fix(php): restore the visitor docblock prose dropped as collateral by the generic-visitor refactor in v0.22.33 -- the parameter's own rustdoc summary, the list of values the callback may return, and what the default implementations return. All three come from the IR (the context type's doc, the result enum's variants, `default_result_expr`) rather than the hardcoded type names the refactor correctly removed, and the default-implementation sentence is gated on asking the trait whether every method has one.
- fix(verify): report a vendored `schemas/alef.schema.json` that has fallen behind. The file is alef's own `alef.toml` schema, vendored by consumers for editor validation; nothing wrote it, and `alef verify` structurally could not see it, since JSON cannot carry the `alef:hash:` marker its walk keys on. The verdict is split rather than byte-exact: a copy describing a different config surface fails the run, while one differing only in version stamp or formatting is informational -- because the version is embedded in three root keys, so every alef release makes every vendored copy byte-stale whether or not the config surface moved, and a gate that reddens on every release is one operators route around.
...
- feat(config): `[workspace.ownership] user_owned`, declaring paths the consuming repository authors rather than alef. `alef verify` already treated create-once seeds this way; the write path never asked, so four stages passing `overwrite: true` refused them on every run and reported a failure whose remedy (`alef adopt`) the same tool correctly refuses. Declared paths are never overwritten at any flag setting, are seeded once if absent, carry no provenance marker, stay in `expected_paths` so the orphan sweep cannot delete them, and are reported as a counted disposition rather than folded into the refusal tally. A malformed pattern is a hard error: a silently dropped one costs a consumer their file.
- feat(e2e): `[crates.e2e.snippets].mock_only`, retiring the reserved-domain warning for a corpus whose fixtures are served by a mock rather than a public address. Suppression is scoped by the fixture's own source: a fixture that inherited the corpus base is silenced, while one that declared `docs.sample_url` and failed to resolve is still reported, and `mock_only` alongside `sample_base_url`/`sample_url_template`/`sample_url_manifest` fails the run -- without that rejection a genuinely missing URL could be silenced by a coexisting template.

- feat(cli): `bin_cli::build_info::build_identity()`, one build-identity source shared by the `--version` banner and every cache key, so the two cannot disagree about which alef is running. A clean or git-less build's identity deliberately excludes the build time, keeping released binaries' caches warm and shareable across runs; only a dirty tree -- where the commit does not determine the source -- folds it in.
- feat(e2e): `FieldResolver::element_accessor`, which renders a path already relative to a bound collection element instead of re-anchoring it against the call's result type. `accessor` and `element_accessor` now share one renderer and differ only in the anchor.
- feat(e2e): `e2e::codegen::fixture_refusal`, a thread-local ledger for consumer-fixable refusals recognised inside expression builders that cannot return a `Result`, drained at `E2eCodegen::generate_gated` and at the documentation-snippet render boundary.
- test(deps): `every_unmarked_version_const_is_a_known_and_reasoned_exception` fails when a new `pub const` version in `src/core/template_versions.rs` ships with no `// renovate:` marker and is not on the reasoned allowlist, and equally when an allowlisted unmarked const has since gained a marker so the stale entry cannot mask the next regression.

- feat(e2e): `[crates.e2e.snippets].sample_url_manifest` (`path`, `variable`), supplying `sample_url_template` placeholder values from an external JSON manifest keyed by each fixture's own `docs.body_file`, so a content-addressed corpus does not need every digest hand-declared per fixture. Manifest values and a fixture's own `docs.sample_url_vars` merge through the existing resolver, with the fixture's explicit entry winning. A configured manifest that is missing, unreadable, malformed, or carries a duplicate or non-string entry fails generation naming the file, never falling back silently. A fixture the manifest does not cover still falls back to `sample_base_url` and still raises the reserved-domain warning, so this can never silence that warning wholesale.
...
- fix(cli): fail generation when a committed `Cargo.lock` beside a generated `Cargo.toml` cannot satisfy it. The relock hook only runs where alef's own manifest bytes changed, so a requirement that moved in a path dependency alef does not generate left the lock stale and the run exited 0 over a crate that no longer resolves (`cargo metadata --locked` failing on a transitive version bump). The check is deliberately one-sided -- a dependency is reported only when its name IS pinned and no pinned version satisfies the requirement, never when it is absent -- so it guards against a false green without manufacturing red on `[patch]`, renames, or platform-gated deps.

- fix(pyo3): pass the Rust default through for a non-`Named` field whose options dataclass defaults it to `None`. `options.py` uses `None` as a sentinel meaning "omit this keyword so the pyclass `#[new]` applies the real Rust default", and `types.rs` declares `T | None` from that same fact -- but the converter honoured it only for `TypeRef::Named`, so a `Vec<String>` field whose Rust default comes from a function call had the sentinel passed straight through as `_rust.T(field=None)` and failed extraction with `TypeError: 'None' is not an instance of 'Sequence'`, far from the dataclass that produced it. The omission now follows `admits_none`, the single fact the type hint is derived from. The pure-Python attribute still reads `None` until constructed through the binding; that is unchanged and is the sentinel working as designed.

- fix(e2e,swift): resolve the JSON-bridge verdict against the leaf's real owner type. `not_empty` on a genuine `Vec<T>` degraded to a bare `!= nil` because a crate-wide bare-leaf-name index marked the name bridged whenever ANY type in the crate declared a field of that name as `Option<Vec<T>>`. The bridged getter is non-optional, so the emitted comparison could not fail -- an assertion that read as coverage while passing for an empty collection, and for an absent one.
- fix(e2e,swift): refuse `count_min`/`count_equals` on a JSON-bridged leaf instead of counting the characters of the JSON text. `swift_count_target` wrapped such a leaf in `.toString()`, so a `count_min: 1` against an empty collection compared `"[]".count >= 1` -- `2 >= 1`, a pass on exactly the input the assertion exists to reject. Two integration tests pinned that shape as correct on the (accurate) grounds that the getter returns `-> String`; the shape is type-correct and semantically empty, so both now pin the recorded skip.
- fix(e2e,swift): stop an `exclude_fields` entry dropping same-named fields on unrelated types. The type-blind name fallback ran first and unconditionally, so one `"Archive.entries"` entry voided every assertion whose path merely contained `entries` -- reported as `ExcludedFromSwiftBinding`, which reads as a deliberate config decision rather than the misclassification it was. The fallback now runs only when the type walk cannot reach every segment, and the walk itself is no longer starved of its anchor: `with_swift_root_type` assigns unconditionally, so a fixture with no explicit `result_type` override had `build_swift_first_class_map`'s own answer erased and could never reach a single segment.
- fix(e2e,rust): stop emitting a member access on a result type that has no members. A call declaring `result_is_simple` short-circuited the field-availability guard, and `render_not_empty_assertion` -- the one Rust renderer that rebuilds its own accessor -- then re-derived `result.<field>` against a byte payload, `error[E0609]`, failing the whole generated e2e crate. Sixteen of seventeen backends already skipped this correctly; Rust now asks the same `FieldResolver` authority and records the drop through the existing `FieldSkip` funnel instead of deriving the rule a second time.
- fix(codegen): report a foreign cfg-gated enum variant once per run rather than once per backend and conversion direction. One such variant produced up to fifteen identical `WARN` lines on every clean regen -- from eight backends and three shared codegen modules -- so a consumer using `[[crates.source_crates]]` saw a permanent wall of warnings restating one fact. The run-level `codegen::foreign_cfg_variants` pass emits it once per `(enum, variant)`, keyed off the single `ApiSurface` walk so no shared mutable state is involved; the per-generator detail moved to `DEBUG` under the same field names.
- fix(wasm): declare `serde-wasm-bindgen` in the generated crate's `[package.metadata.cargo-machete] ignored` list. The dependency is emitted for every wasm crate but only referenced when a field, trait bridge or return value actually bridges through it, so a consumer whose surface never triggers that reference had `cargo-machete` flag it -- and, the manifest being alef-generated, nothing to fix on their side. Every other unconditionally-declared dependency (`futures`, `js-sys`, `serde`, `serde_json`, `wasm-bindgen-futures`, and `tracing` when trait bridges exist) was already listed; this one was simply omitted. The regression test asserts the general rule -- every declared registry dependency the generated Rust never mentions must be ignored -- rather than pinning the one name.

- fix(swift): stop the public Swift `enum`'s `case` list advertising a foreign cfg-gated variant the swift-bridge mirror enum drops. `emit_enum`/`emit_enum_without_into_rust` walked `EnumDef::variants` with no cfg awareness while the mirror declaration and its to-string match both filtered through `enum_variant_declaration`, so the facade offered a case no conversion could produce (the mirror's match never emits it) or accept (its `from_swift_string` arm is dropped unconditionally). Both surfaces now ask one `gen_rust_crate::enums::declared_variants`, resolved against `enabled_features_for_language(config, Language::Swift)` -- the narrower set the bridge crate itself uses, not the host-widened `effective_swift_codegen_features`, which answers a different question and would have left the two disagreeing.
- fix(dart): stop a `style = "ffi"` Dart `enum` advertising a foreign cfg-gated variant the linked C library rejects. `gen_ffi::types::emit_enum` now filters through `enum_variant_declaration` against the `Language::Ffi` feature set -- the same authority and the same set `ffi::gen_bindings::types::declared_variant_indices` uses to build the `{prefix}_{enum}_from_i32`/`_from_str` validators. `DartStyle::Ffi` emits no Rust crate of its own, so the FFI crate is the only Rust side it has; asking with the Dart set instead would trade this over-exposure for an under-exposure of host-owned gated variants.
- fix(e2e,kotlin): fill an omitted optional DTO argument from the declared parameter rather than a constructor the generator cannot verify exists. A `json_object` arg marked optional with no fixture value emitted `OptionsType()` for kotlin_android and `OptionsType.builder().build()` for the JVM target unconditionally, but the first compiles only for a type in `default_constructible_type_names` and the second only for a record `should_emit_builder` gave a factory, so snippets failed with `No value passed for parameter 'x'` or `unresolved reference: builder`. When the IR declares the parameter `Option<T>` both emitters already render it `name: T? = null`, so the builder now asks `TargetParams::declares_param_optional` -- the same `ParamDef::optional` field they read -- and passes `null`, needing no constructor at all. `f34fefff9` added that consultation to the scalar branch only.
- fix(e2e,node): construct declared binding classes inside handle-config lists in the TypeScript generator. The class map was consulted only for a directly nested object, so an object inside a list fell through to the untyped literal dump and emitted a plain object where wasm-bindgen requires a class instance -- even though `derive_nested_types_for_wasm` had already unwrapped `Vec<Named>` into that map. One recursive traversal now owns the value at every depth, and the import collector runs that same traversal with its output discarded so the emitted body and the import block cannot name different classes.
- fix(e2e): scope the documentation-snippet stage to `--lang`. `languages_or` treated the narrowed set as a fallback, so a configured `[crates.e2e.snippets].languages` discarded it and a single-language run rewrote every language's snippet tree. The shared coverage ledger is still computed over the unfiltered configured set -- narrowing it would drop every unnamed language's ownership entry and leave the manifest permanently stale against `alef verify`.
- fix(e2e): apply `handle_nested_types` to objects nested inside a list. The map was consumed at depth 0 and list elements were rendered by an untyped literal renderer, so each element stayed a bare dict and the generated pyclass rejected it at runtime. One recursive traversal now serves both the renderer and the import collector, which were previously two independent walks that could disagree about which classes to import.
- fix(e2e): stop appending an unannotatable `.as_ref()` to a Rust streaming assertion whose accessor already pinned its own type. `is_optional` answers about the declared IR field while the emitted expression had already flattened the `Option` away via `.iter().flatten()`, so `Vec<T>`'s several `AsRef` targets left `T` unconstrained (`error[E0282]`). The append is now gated on the one accessor shape that really yields an `Option` place expression.
- fix(wasm): stop generated TypeScript snippets referencing enum members the WASM binding never declares. An externally tagged enum carrying a payload variant is emitted by the backend as a struct with no members at all, while the snippet builder still classified it as a C-style enum; it also re-cased a serde wire value into a host identifier, which diverges under `rename`/`rename_all`/acronyms.
- fix(snippets): key the cargo, Go and Zig toolchain caches on session configuration rather than the working-tree fingerprint. Any source edit discarded a multi-gigabyte compiler cache and stranded it, and nothing had ever swept `.alef/snippets/cache/` -- measured at 19.9 GiB across five retained directories in one consumer repo. Compilers track their own artifacts' staleness; the verdict cache still rekeys on content, so a stale verdict is never replayed.
- fix(snippets): reclaim stale toolchain cache directories, retaining every key the current run uses plus the most recently used generations, and never reclaiming one modified inside the run-timeout grace window so a concurrent run's live target directory cannot be deleted mid-build.
- fix(napi): stop the public `index.d.ts` overlay and the forced `ts_type` union from advertising a foreign cfg-gated enum variant the generated Rust enum omits. Both re-derived their variant list from `EnumDef::variants` directly instead of consulting `enum_variant_declaration`, the authority the `From` impls already use, so a consumer could name a value that can never be constructed and type-check clean. `gen_dts` also index-zipped names against values, which would have misaligned them once the lists could differ in length.
- fix(swift): stop generated files under a SwiftPM target importing the very module they belong to, which Swift reports as `file ... is part of module '<M>'; ignoring import`. The decision is now made from each file's destination path, so the same import stays correct in files emitted into other modules.
- fix(swift): call a `Void`-returning bridged trait method as a bare statement instead of binding `let result = try ...`, which warned twice (unexpected `Void` inference and unused variable) and failed a warnings-as-errors build. The success-body emitter already treated `TypeRef::Unit` as producing nothing, so the two halves of one decision disagreed.
- fix(e2e): attribute kotlin_android skips, inert refusals and unacknowledged-gap verdicts to `kotlin_android` rather than `kotlin` on the shared skip ledger. `should_skip` compares canonical language names and `kotlin_android` is not an alias of `kotlin`, so a fixture declaring `"skip": {"languages": ["kotlin_android"]}` was not honoured on the Android run and became a hard strict-mode failure naming a gap the consumer had already acknowledged.
- fix(e2e): render wildcard element closures relative to the element binding in the remaining nine backends -- go, python, elixir, typescript, java, csharp, kotlin, php and swift (two independent sites). All nine route the element half through the result-anchored `FieldResolver::accessor`, whose envelope rescue re-anchors against the call's result type; that rescue is language-independent, so a consumer survey finding some backends clean reflected the corpus, not the code. kotlin renders index 0 as `.first()` rather than `[0]`, so a grep-based survey reports it clean while it is defective. typescript's wildcard branch precedes every wasm rewrite and hardcodes one accessor language, so its node and wasm trees are fixed together. `r` and `ruby` make the same call and are immune only because neither wires `with_ir_result_fields`, so they become defective the moment they anchor a root type.
- fix(cli): stop reporting a wholly absent previous-run baseline as a per-backend bookkeeping defect. `clear_cache` removes `.alef/` outright, so `alef all --clean` leaves no `*-ownership` manifest and every generated output root reported `0 manifest entry(s)` in one burst -- read as 20+ independent backend defects when it is one missing baseline that the next converged run repopulates. The disk-scan loop now distinguishes "no baseline anywhere" (debug, with the self-heal named) from "baseline exists but does not cover this root" (unchanged warning, now also reporting how many paths the baseline did record). The refusal behaviour is identical either way: an unvouched root is still never scanned. `ba58517b1` made this same split in the sibling `allowed_roots` loop and left this one standing.

- fix(swift): declare a `Result`-wrapped return in a type method's bridge extern block whenever a fieldless enum parameter forces the shim to be fallible. `emit_extern_block_for_type_methods` computed its declared return type from `method.error_type.is_some()` alone -- the same incomplete check `2190ce44c` fixed for free functions -- while `emit_type_method_shims` forces the shim's own return to `Result<_, String>` whenever a unit-enum parameter's wire-string reconstruction can fail, so swift-bridge rejected the declaration/implementation mismatch with `error[E0308]`. Both sides now call one `forces_fallible_enum_bridge` helper instead of each deriving the rule, so the four call sites that previously hand-copied it cannot drift apart again.
- fix(e2e): stop the Rust e2e generator emitting a wildcard (`container[].field`) membership closure whose body re-applied the container path to the element binding (`|e| e.container[0].field`), producing `error[E0609]: no field` against the element type and breaking compilation of the generated e2e crate. The element half of the path was being rendered by the result-anchored `accessor`, whose envelope rescue correctly re-anchors a result-relative path and is precisely wrong for a path already split off its container.
- fix(jni): collapse three independently-maintained copies of the JNI package precedence chain into one. `core::jni::jni_package` had its own fallback (`com.example.{name}`) that diverged from every other resolver in the tree (`unconfigured.alef`, via `ResolvedCrateConfig::kotlin_package`), so one generated `-jni` crate could emit two different package prefixes on its own `Java_*` symbols depending on which path produced them.
- fix(snippets): salt the snippet-validation cache key on the full build identity `alef --version` reports rather than the alef semver alone. Every release candidate in a cycle shares one version string, so two different binaries shared cache entries and replayed each other's verdicts, masking both a validator fix shipped between them and a breakage introduced between them.
- fix(cli): fold the same stamped build identity into the `.alef/` IR, per-language and per-stage cache keys, which had the same blind spot narrowed only by a best-effort `current_exe` mtime probe that contributes nothing when it fails.
- fix(cli): treat a manifested stage output carrying alef's provenance marker but no `alef:hash:` line as a cache miss rather than agreement. A run that ended between the write pass and `finalize_hashes` left such files claimed-but-unstamped, and the next run's `is_stage_cached`/`is_lang_cached` hit skipped the stamping pass that would have fixed them, so `poly fmt` kept reformatting the file and the next alef write kept putting its own bytes back. Repo-root scaffold files (`.cargo/config.toml`, `rust-toolchain.toml`, `poly.toml`, `rustfmt.toml`) sit outside every `generate_sweep_roots` root and had no other route back to a stamp. Unmarked outputs -- `generated_header: false` create-once seeds, and formats with no comment syntax such as `.json` and `.jar` -- keep the existence-only rule unchanged.
- fix(e2e): stop a fixture key the resolved options type does not declare from panicking the TypeScript/node and wasm e2e generators. The refusal now travels as that backend's own `anyhow::Error` through `E2eCodegen::generate_gated`, so `run_generators` isolates it, sibling backends still generate, and `alef all` continues with its remaining stages instead of aborting the process at exit 101 and silently skipping README, docs and snippet validation.
- fix(e2e): name the fixture, the call (`[e2e.calls.<name>]`) and the language in the undeclared-fixture-key diagnostic, and point at the per-language `options_type` override as the resolution lever. When the type was inherited from a file-level `[e2e.call.overrides.<lang>].options_type` default rather than declared for the call, the message says so and directs the fix to `[e2e.calls.<call>.overrides.<lang>]` instead of the shared default. The previous text named only the fixture and the Rust struct as remedies, which misdirects whenever the real cause is a mis-resolved `options_type`. All undeclared keys in one fixture object are now reported together instead of only the first.

- fix(codegen): keep the `binding -> core` enum conversion catch-all for a foreign cfg-gated variant that this binding's own declaration still emits. `emit_cfg_gated_arm` drops such an arm unconditionally -- a wrapper crate cannot forward a foreign crate's feature as its own gate -- while the catch-all was suppressed whenever the configured feature set proved the variant unreachable. That proof describes only the real core type; the enum wrapper declarations for pyo3, magnus, rustler, dart and wasm keep the variant regardless, so the match lost its arm AND its catch-all: `error[E0004]: non-exhaustive patterns` in the generated crate. Adds `ConversionConfig::declaration_drops_unreachable_foreign_variants` (default `false`) so the conversion asks whether the declaration agrees the variant is gone rather than re-deriving reachability from a second copy of the proof. napi, the one backend whose declaration does drop it, opts in and still emits no catch-all -- emitting one there would be `unreachable_patterns`. The `core -> binding` direction is deliberately unchanged: that match is over the real core type, where the feature proof is already complete.
- fix(php,swift,ffi): stop advertising a foreign cfg-gated enum variant that this binding's configured features prove unreachable. PHP dropped it from the `#[php_impl]` class constants, the PHPStan stub and the flat-data-enum `from_json` `allowed_tags`; Swift from the swift-bridge mirror declaration and its `to_string` match; the C ABI from the generated `_from_i32`/`_from_str` validators, matching what the paired `_from_i32_rs` helper already refuses to produce. Kept variants retain their original discriminant, never renumbered. These compiled, but exposed a value the conversion could never construct.
- fix(csharp): emit `new T()` rather than `default(T)` for a visible `Named` return in a generated trait-bridge stub. `default(T)` is `null` for any C# class, so a non-nullable reference return failed CS8603.
- fix(docs): render an enum field's documented default in each backend's real generated syntax for Go, Java, Swift, Dart and PHP. One generic `EnumType.Variant` shape named a constant that does not exist in any of the five -- Go concatenates with no separator, Java keeps the raw variant identifier, Swift and Dart lower-camel-case it, PHP upper-cases the whole name. The docs now ask the same naming authority the backend emits through.
- fix(snippets): match Kotlin compile tasks with Groovy's `=~` (find) instead of `==~` (full string match) in the Gradle classpath init script. Kotlin Multiplatform names per-target tasks with the target after `Kotlin` (`compileKotlinJvm`, `compileKotlinIosArm64`), so a full match silently skipped every one of them and every dependency reachable only through that task. Also rejects a resolution that matched compile tasks but produced no library entries for any of them -- every real Kotlin compilation has kotlin-stdlib at minimum, so that shape is a resolution defect, not a small project, and was previously accepted silently.
- fix(cli): stop `alef adopt` re-offering a file the write guard already owns. `classify` decided `AlreadyOwned` from the alef marker alone, never consulting the `is_owned_by_ownership_record` union the write guard and `classify_binary` already use, so an unmarkable already-owned path such as `.alef-snippet-coverage.json` was re-listed for adoption every run.
- fix(e2e): resolve a tagged-union crossing whose payload type declares another enum-typed field that is itself a declared crossing. The previous fix resolved one crossing then handed the remaining suffix to a struct-only walk that could not advance through a second enum boundary, so a leaf reached through two chained crossings was dropped from generated doc snippets even with both crossings declared. The walk is now recursive with no depth or count limit.
- fix(cli): relock a nested binding-crate `Cargo.lock` (for example `packages/ruby/ext/<gem>_rb/native/Cargo.lock`) whenever `alef build`, `alef generate` or `alef scaffold` actually rewrites the alef-owned `Cargo.toml` beside it. The only pre-existing relock ran from the version-bump pipeline alone, but a nested manifest is `generated_header: true` and gets rewritten on every ordinary build, so a dependency requirement that moved with a template bump left `cargo check --locked` failing against a pin the manifest no longer satisfies, with no version bump anywhere in sight. Scoped to the manifests a given write actually changed, never a full-tree walk, and best-effort like the pipeline's other lockfile refreshes: the relock runs `cargo update --offline`, so a version absent from the local registry cache still warns rather than aborting the build, naming the lockfile and the manual `cargo update` needed.
- fix(snippets): union each project's `*RuntimeClasspath` configurations into the resolved Kotlin snippet classpath, leniently, for any project with a matched Kotlin compile task. A compile task's `libraries`/`classpath` mirrors `compileClasspath`, which by Gradle's own `api`/`implementation` scoping hides an `implementation`-scoped transitive dependency of a declared library -- a real artifact the module ships with and that resolves fine on `runtimeClasspath`. Snippets referencing such a dependency failed as unresolved references while the project's own build succeeded, and the existing completeness guard never fired because every other entry was still present. Runtime entries are filtered to jars and directories, since an Android runtime classpath can report `.aar` archives kotlinc cannot read.
- fix(kotlin_android): read `alef.skipHostJni`/`alef.skipAndroidJni` through `providers.gradleProperty(...)` rather than the `project.properties[...]` map, which Gradle deprecated for removal in 10.
- fix(kotlin_android): drop the legacy `package` attribute from the generated `AndroidManifest.xml`; `android { namespace = ... }` in the generated `build.gradle.kts` is already the namespace source current AGP accepts.
- fix(kotlin): give the generated `@JsonProperty` on data-class constructor properties an explicit `@param:` use-site target, pinning today's implicit target before Kotlin makes it ambiguous with the property and field targets.
- fix(kotlin): replace `JsonNode.fields()`, deprecated in Jackson 2.19, with `properties().iterator()` in the generated heterogeneous default-enum deserializer.
- fix(kotlin,kotlin_android): replace `ObjectMapper.setSerializationInclusion(...)` with the non-deprecated `setDefaultPropertyInclusion(...)` in the generated facade and value-method Jackson mappers.
- test(dart): add table-driven coverage asserting the Dart mirror enum declaration and both conversion directions agree about foreign cfg-gated variants across enum shapes -- single and multiple gated variants, proven, unproven and unknown reachability, host- versus foreign-owned enums, and the orthogonal `excluded_variants` gap. An audit of the `E0004` fix against a report of drift across several enums found no residual Dart gap: both directions route every enum through the one shared resolver, so the fix was already systemic rather than scoped to the enum that motivated it.
- test(gate): give `generated_output_downstream_gate` a genuinely foreign cfg-gated enum variant, merged from a sibling crate through `[[crates.source_crates]].roots`, and two sabotage-backed lanes over the emitted pyo3 crate: one strips the `binding -> core` catch-all and requires `error[E0004]: non-exhaustive patterns`, the other injects a redundant catch-all into the already-exhaustive `core -> binding` match and requires `unreachable_patterns` under `-D warnings`. The gate already compiled emitted Rust, but no fixture had ever declared a variant the emitting crate cannot gate on, so the one shape that shipped a non-compiling generated crate past a fully green suite was structurally invisible to it. Both directions are covered deliberately: they are opposite failures, and fixing one has historically caused the other.
- chore(deps): refresh the dependency versions alef emits into generated bindings, verified against each ecosystem's own registry rather than assumed. npm (`@napi-rs/cli`, `typescript`, `vitest`, `@types/node`, `rollup`, both vite plugins), pub.dev (`test`, `http`, `crypto`, `json_annotation`, `build_runner`, `json_serializable`), `checkstyle` 13.11.0 -> 14.0.0, the Android Gradle Plugin 9.2.1 -> 9.3.2, `Microsoft.NET.Test.Sdk`, `pyrefly` and CRAN `rextendr`. Every Rust crate pin was already current; the Ruby and Elixir/Gleam pins are two-segment ranges that already admit their package's latest release, so they are deliberately unchanged rather than churned.
- chore(deps): raise the generated Dart SDK floor to `>=3.13.0 <4.0.0` and the Gradle wrapper to 9.7.1. The SDK floor is a BREAKING change for consumers building on Dart 3.11.x or 3.12.x. AGP 9.3.x requires Gradle >= 9.5.0, so the two moves are compatible; `ANDROID_JVM_TARGET` deliberately stays at 17, which is what AGP itself requires, rather than following `JAVA_JVM_TARGET`.
- fix(deps): repair two version pins that looked tracked but never were. `PHPUNIT` and `GUZZLE` carried `// renovate:` markers with prose wedged between the marker and the const, which breaks the customManager's required line adjacency — the same silent failure the module already documents elsewhere. Adds markers for the Android Gradle Plugin, both AndroidX test artifacts, three Gleam ranges, the Gradle wrapper and the Zig floor, with the registry overrides they need: AGP and AndroidX resolve from Google's Maven repo, not Maven Central, so without an explicit `registryUrls` their markers would resolve to nothing and go on looking tracked.
- fix(deps): widen rather than replace the deliberately multi-clause constraints. Under the default replace strategy Renovate rewrites an `||` or `and` chain to its last clause alone, which would silently collapse `^11.5 || ^12.0 || ^13.1` to a single major and drop the lower bound from every Gleam range. `MAVEN_CORE`, `RB_SYS` and the Ruby gem pins stay deliberately untracked, each for a recorded reason.
- fix(snippets): typecheck a hand-written TypeScript snippet that imports a Node builtin. The isolated session runs in an OS temp directory and never sees the consumer's project, so their installed `@types/node` was invisible to it. The session's scratch now nests under the nearest ancestor of the snippet's own file that actually has `@types/node` installed, and names `"types": ["node"]` in its tsconfig only when that ancestor was found -- naming it unconditionally produces `TS2688` for every consumer without the install. Relocating the scratch alone is NOT sufficient: the `tsc` this resolves from `PATH` may be the native compiler (7.x), which does not perform the automatic `@types` acquisition the classic compiler does, verified by hand against both generations. With nothing resolvable the behavior is byte-identical to before, so a snippet that cannot resolve its typings still fails loudly with tsc's own diagnostic rather than being silently downgraded.
- fix(kotlin): stop passing `null` for a Kotlin parameter the target declares non-nullable. The e2e and snippet argument builder substituted a bare `null` whenever the fixture's own `optional` flag said a field may be absent -- a hand-authored claim about the FIXTURE, unrelated to whether the target actually declares the parameter optional. Java tolerates that because its boxed types accept `null` syntactically; Kotlin's non-nullable primitives do not, so generated snippets failed to compile. Kotlin's generators had never opted into the IR-aware `TargetParams` seam that TypeScript, Java, C#, Swift and Go already use for exactly this class of disagreement; they now consult the same declared-optionality authority the signature generator itself checks, and emit a type-appropriate literal rather than `null` when the target genuinely requires the argument.
- fix(swift): declare a `Result`-wrapped return in the bridge whenever a fieldless enum parameter forces the shim to be fallible. swift-bridge splits every function into an `extern "Rust"` declaration and the `pub fn` that implements it, and the two computed their return types independently: the shim already knew that a unit enum crosses the boundary as a wire string and is reconstructed with `?`, so an otherwise-infallible function became `Result<_, String>`, while the declaration looked only at the function's own `error_type` and declared the bare type. swift-bridge's macro checks one against the other, so the emitted crate failed `E0308`. The declaration now consults the same unit-enum set the shim trusts rather than re-deriving a second, incomplete answer. Not specific to a foreign or cfg-gated enum -- any fieldless enum parameter on a function with no error type of its own reproduces it; a dependency-owned enum was simply the first shape to pair the two.
- fix(snippets): declare the `Buffer` global in the TypeScript session's ambient declaration. The base64 branch of the generated bytes expression emits a bare `Buffer.from(value, "base64")`, which the declaration covered for `node:fs/promises` but not for `Buffer`, failing `TS2591` in alef's own generated snippets.
...
- fix(csharp): four defects in generated doc snippets — a fixture-authored `iterate` item name is now disambiguated against the names already bound in the snippet's scope (CS0136); an optional collection consumed by a `foreach` gets the null-forgiving operator at the point of consumption rather than in the shared accessor (CS8602); a `bytes` struct field resolves its `u8` element type and renders `new byte[] { }` instead of falling through to an untyped `List<string>` (CS0029); and a trait-bridge stub returning a non-nullable type no longer returns a bare `null` (CS8603).
- fix(swift): normalize the materialized swift-bridge trio before writing it. `PostBuildStep::MaterializeSwiftBridge` wrote swift-bridge's own output through a raw `fs::write`, the one path in `alef build`/`all`/`generate` that bypassed `normalize_content`, so whatever trailing whitespace that external codegen emitted shipped uncorrected into every consumer tree.
- fix(cli): fold a post-build step's `owned_paths()` into `alef all`'s ownership bookkeeping, as `alef generate` and `alef verify` already did. Without it the materialized swift-bridge files were written every run but never recorded, so the next run's previous-paths manifest read empty while the current run recorded kept files -- the asymmetry that blocks safe stale-output reclamation.
- fix(snippets): invalidate a cached snippet verdict when the snippet's own annotation or the `docs.snippets` side-effect policy changes. Neither was part of the cache identity, so a corrected snippet kept reporting its old downgrade -- and, in the dangerous direction, a newly broken one would keep reporting a cached pass -- until an explicit `alef cache clear`.
- fix(process,kotlin): detect a stopped (`STAT=T`) child instead of waiting on it forever, and stop triggering the stop in the first place. The Gradle classpath child inherited the terminal in a background process group, took `SIGTTIN`, and never resumed -- `SIGCONT` returned it immediately to stopped. Redirect its stdin from `/dev/null` to remove the trigger, bound the post-kill reap, and treat a child continuously stopped for a grace period as expired. The Kotlin classpath fallback also now globs variant subdirectories, covers AGP 9's `built_in_kotlinc` output and the project's own `libs/`, instead of reporting every generated snippet unavailable.
- fix(napi): emit one `Default` candidate per declared enum variant, each guarded `all(<own cfg>, not(any(<earlier cfgs>)))`, instead of a single impl gated on only the first variant's feature. The enum type itself carries no cfg, so any feature combination enabling the enum without that first variant left a `derive(Default)` with no impl -- a build failure under `--no-default-features --features <later>`.
...
- Added `crate::test_support::assert_elapsed_under` and routed the crate's tight wall-clock-bounded test assertions through it, so a future transient failure under machine load reports its measured elapsed time, asserted bound, and margin instead of requiring another audit to identify.
- Added `crate::test_support::REAL_MVN_LOCK`/`RealMvnGuard` and `maven_local_repo_dir` so the three real-`mvn` scaffold bite tests share a per-worktree Maven local repository (`target/mvn-repo-cache-test`) instead of the developer's ambient `~/.m2`, closing the cross-worktree contention hazard without paying a cold plugin download on every run.
- test(pyo3): add a sync-function regression case to `plain_function_dataclass_return_conversion_tests.rs` proving `emit_function_return_call`'s native-to-public-dataclass conversion applies with no `await`, not only on the async path already covered by `eb077bb5d`'s fix for the plain-function return-value conversion gap
- `alef generate`'s post-build failure for one language (`complete_generated_artifacts`) used to `return Err` immediately, skipping the crate's single terminal `finalize_hashes_sweeping` stamp entirely -- every language's freshly-written binding/service-API/public-API/stub output that run, not just the failing language's, shipped with its `// This file is auto-generated by alef` header and no `alef:hash:` line, permanently (a later run with unchanged content is a cache hit that never re-enters the write path). Deferred into the same `StageFailures` type `alef all` already used for the identical hazard (task #186); both commands now finish writing, formatting and stamping every crate before reporting any post-build failures.
- `alef generate`'s terminal stamp now sweeps each crate's own language/package roots (`finalize_hashes_sweeping`, matching `alef all`) instead of only re-stamping the paths this run's in-memory generation tracked, so a file a per-language cache hit or an interrupted prior run left marker-carrying but unstamped on disk gets healed from its own on-disk content on the next `alef generate` too.
- fix(rustler): emit a narrow `#[expect(clippy::large_enum_variant, reason = "...")]` on generated `NifTaggedEnum` data enums whose heaviest variant's estimated size substantially exceeds its lightest siblings', instead of leaving `-D warnings` clippy gates blocked in consumer Elixir NIF workspaces
- add `codegen::enum_variant_size::enum_should_expect_large_variant_lint`, an IR-only, deliberately conservative size heuristic (biased toward false negatives, never false positives, to avoid `unfulfilled_lint_expectation` hard errors) that gates the attribute per-enum rather than emitting it unconditionally
- Added a narrow, documented warning-acknowledgement mechanism (task #540): `crate::core::warning_ack::AcknowledgementLedger` matches acknowledgements by exact warning identity plus exact source target, fails the run when a configured acknowledgement matches nothing, and reports a nonzero matched count instead of suppressing silently.
- Added `AcknowledgeableWarningCategory`, a closed enum naming only the warning categories a consumer may acknowledge; categories that must stay always-actionable have no variant and so cannot be expressed in `alef.toml` at all.
- Added `[[crates.e2e.snippets.acknowledged_warnings]]` config entries (`category`, `identity`, `target`, optional `reason`) so a project can silence a specific fixture/target-language pair publishing the reserved documentation placeholder domain (`example.com`), without silencing the category wholesale.
- The reserved-placeholder-domain warning now names, for each unacknowledged fixture/language pair, the exact `alef.toml` entry that would acknowledge it, and a separate run summary reports how many warning occurrences were acknowledged.
- fix: anchor `FieldResolver`'s IR-derived enum map (`with_ir_enum_map`) in the shared docs-snippet presentation resolver (`e2e::codegen::presentation`), so a field reached only through a `fields_method_calls`-declared tagged-union crossing survives trimming and appears in generated doc snippets across every language backend, instead of being silently dropped at the union boundary.
- fix: `alef.toml` `fields_optional`/`fields_array` validation (`e2e::validate::validate_field_classifications`) now consults the crate's `EnumDef`s and the consumer's own `fields_method_calls` declarations before reporting a leaf as "unverified", so a declared and IR-confirmed tagged-union crossing no longer emits a false warning; an undeclared or IR-unconfirmed crossing still warns.
...
- test(e2e): acquire `RealCargoGuard` in `docs_nested_bytes_generated_rust_compiles`, the one test that spawns a real `cargo check` without serializing against cargo's machine-wide package-cache lock
- fix(napi): route `gen_tagged_enum_core_to_binding`'s catch-all decision through `enum_conversion_needs_catch_all_for_features`, threading the crate's configured Node feature set through instead of a feature-blind `has_cfg_variants` check
- fix(extendr): route `enum_conversions::gen_from_binding_to_core`/`gen_from_core_to_binding`'s catch-all decision through `enum_conversion_needs_catch_all_for_features`, using a dedicated configured-feature computation distinct from the existing host-only `effective_r_cfg_features`
- fix(dart): route `enum_conversions::emit_from_impl_for_enum`/`emit_from_mirror_to_core_enum`'s catch-all decision through `enum_conversion_needs_catch_all_for_features`, which also fixes a pre-existing host-vs-foreign blindness in the same decision (the Dart bridge crate blanket-allows `unreachable_patterns`, so this was not previously an observable build failure)
- fix(swift): route `enums::emit_enum_wrapper`'s catch-all decision through `enum_conversion_needs_catch_all_for_features`, which also fixes a pre-existing host-vs-foreign blindness in the same decision (the Swift bridge crate blanket-allows `unreachable_patterns`, so this was not previously an observable build failure)
- fix(tests): make `strip_crate_level_allows` in `ffi_allowlist_gate.rs` strip multi-line `#![allow(...)]` blocks, not just single-line ones, so the FFI dead-clippy-entry gate actually removes the crate-level allow it is probing instead of leaving a syntactically valid attribute in place that silently overrides the test's `-W` override
- test(ffi): retarget `dead_entry_check_catches_a_planted_orphan`'s planted orphan from `clippy::collapsible_if` (made genuinely dead by the let-chain fix in `8893f7550`) to `clippy::missing_safety_doc`, which still fires against the gate fixture's constructor and free-function wrappers
- Remove the unconditional pre-flight warning `alef all` emitted whenever `docs.snippets.validation_level` was `typecheck`/`compile`/`run`, regardless of whether the artifacts it needs actually existed
- Report `docs.snippets` unresolved-build-artifact results (100% `unresolved_dependency`, no real toolchain gap) at `info` instead of `warn` in `enforce_snippet_summary`, so a normal `alef all`/`alef docs` run stays warning-clean for a `validation_level` that is correct for `alef snippets check`; any admixture of a genuine failure or toolchain gap still warns
- Add `alef snippets check --level <syntax|typecheck|compile|run>` to explicitly override `[docs.snippets].validation_level` for one invocation, independent of what `alef all`/`alef docs` use
- Record a per-crate "generation in progress" marker under the gitignored `.alef/<crate>/` cache before `alef generate`/`alef all` make their first mutation, and clear it once that crate's run records its generation-inputs baseline successfully, so an interrupted `--clean` run (killed, not merely errored) leaves a durable signal instead of a silently partial tree (#268).
- `alef verify` now reports "the last generation run did not complete" for any crate whose marker is still present, distinct from and printed ahead of ordinary staleness/missing-file findings, and no longer folds that crate's missing/frozen/stage-failure findings into the staleness report.
- `alef all`/`alef generate` log a recovery warning when they find a stale in-progress marker for a crate at the start of a run, before regenerating it fully.
- fix(swift): `alef generate`/`alef all` no longer invoke a `cargo build --release` of the swift-bridge crate to materialize `SwiftBridgeCore.swift`/`RustBridgeC.h` — they now run a bare `cargo check`, which runs the same build script (a pure `syn` parse) without the codegen/optimization/linking a release compile performs. `alef build` is unchanged and still compiles in release mode.
- fix(go): derive the trait-bridge vtable C struct name from `ffi_prefix` via the FFI backend's `vtable_struct_name` authority instead of re-deriving it from `crate_name`, which silently diverged whenever `[ffi] prefix` differed from the crate name (#525)
- refactor(go): centralize `{prefix}_free_string`/`_last_error_code`/`_last_error_context` C symbol spelling through `c_symbols`/`c_consumer` in all Go templates and codegen call sites instead of inline `{{ ffi_prefix }}_free_string`-style concatenation, removing a drift risk with no behavior change (#524)
- chore(go): remove the dead `c_type` cast value in the non-optional opaque trait-bridge parameter branch and the five unused `upper_prefix` computations/context entries in the Go service-API generator, neither of which any template referenced
- Thread `configured_features` into `ConversionConfig` at the rustler (non-flat enum), magnus, and pyo3 enum-conversion construction sites, so a foreign cfg-gated enum variant this binding's own feature set proves unreachable no longer leaves behind an unreachable `_ => Default::default()` catch-all under `cargo clippy -D warnings`.
- Add `codegen::conversions::enums::enum_conversion_needs_catch_all_for_features`, a `configured_features`-aware entry point for enum generators that cannot route through `ConversionConfig` at all.
- Fix rustler's flat-data-enum generator (`gen_rustler_flat_data_enum_from_core`) and PHP's tagged-data-enum generator (`gen_flat_data_enum_from_impls`) to delegate their catch-all decision to `enum_conversion_needs_catch_all_for_features` instead of a local `has_cfg_variants` check that ignored the binding's configured features entirely.
- Switch pyo3's fieldless-enum conversion loop from the plain `gen_enum_from_binding_to_core`/`gen_enum_from_core_to_binding` to the `_cfg` variants so `configured_features` actually reaches it.
- Fix the wasm backend's `ConversionConfig` not threading `configured_features` into enum conversions, so a foreign cfg-gated enum variant proven unreachable by the binding's configured feature set no longer leaves behind an unreachable `_ => Default::default()` catch-all that fails `cargo clippy -- -D warnings`
...
- Conftest.py now changes the working directory to `test_documents_dir` even when a fixture set also needs the mock server; the mock-server and file-fixture branches were an if/else, so a fixture needing both silently lost the directory change and file_path/bytes args resolved against pytest's invocation cwd instead of the configured test-documents directory.
- The `os` import is now derived directly from the rendered test-file body (`body.contains("os.")`) instead of from a fixture-level heuristic combined with the body. The heuristic never checked a fixture's declared `env.api_key_var`, so a fixture with an api key var but no mock/client-factory/mock_url/bytes-path arg emitted `os.Getenv(...)` without importing `"os"`.
