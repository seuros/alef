        ...
        && type_path.path.segments.len() >= 2
    {
        ...
        if first_segment == "crate" {
            if let Some(name) = crate_name {
                let mut segments: Vec<String> = type_path.path.segments.iter().map(|s| s.ident.to_string()).collect();
        ...
        match ident.as_str() {
            ...
            "Box" => {
                if let Some(box_inner) = type_resolver::extract_single_generic_arg_syn(seg)
                    && let syn::Type::Path(inner_path) = &*box_inner
            ...
            "Arc" => {
                if let Some(arc_inner) = type_resolver::extract_single_generic_arg_syn(seg)
                    && let syn::Type::Path(inner_path) = &*arc_inner
                    && let Some(inner_seg) = inner_path.path.segments.last()
                {
                    let inner_ident = inner_seg.ident.to_string();
...
pub(crate) fn detect_vec_inner_core_wrapper(ty: &syn::Type) -> crate::core::ir::CoreWrapper {
    ...
    if let syn::Type::Path(p) = ty_ref
        && let Some(seg) = p.path.segments.last()
        && seg.ident == "Vec"
        && let Some(vec_inner) = type_resolver::extract_single_generic_arg_syn(seg)
        && let Some(ident) = outermost_ident(&vec_inner)
        && ident == "Arc"
    {
        return CoreWrapper::Arc;
    }
