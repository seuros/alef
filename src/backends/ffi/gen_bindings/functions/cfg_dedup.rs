...
/// the output; the FFI emitter then declares the same C symbol under multiple cfg branches
/// with disagreeing signatures, which fails loudly (duplicate-definition linker error) instead
/// of silently emitting a signature that is wrong under some cfg combination. ~keep
...
fn warn_on_signature_disagreement(indices: &[usize], functions: &[FunctionDef]) {
    ...
    tracing::warn!(
        ...
        "same-name functions under disjoint cfg gates disagree on signature; skipping cfg dedup \
         for this group so every variant reaches the FFI emitter unmerged rather than silently \
