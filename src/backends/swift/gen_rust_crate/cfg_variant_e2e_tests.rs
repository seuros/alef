...
//! variant run through the REAL `gen_rust_crate::emit` path, not a direct `enums::emit_enum_wrapper`
//! call. Mirrors `backends::wasm::gen_bindings::cfg_variant_e2e_tests`, the pattern task #538
//! established for wasm and task #544 extended to rustler/magnus/pyo3/php.
//!
//! `enums.rs` computed its own `has_cfg_variants` locally, ignoring both host/foreign ownership
//! AND the crate's configured feature set entirely -- so a foreign cfg-gated variant this
//! binding's own configured feature set proves unreachable still produced a trailing
//! `_ => unreachable!(...)` catch-all arm. The Swift bridge crate blanket-allows
//! `unreachable_patterns` at the crate root, so this never failed a Swift build under `-D
...
/// variants -- as FOREIGN. Every enum reachable from the API surface routes through
/// `enums::emit_enum_wrapper` unconditionally (no function needed to force visibility). ~keep
...
fn core_to_binding_conversion(lib_rs: &str) -> &str {
    let start = lib_rs
        ...
        .expect("generated crate must convert the foreign enum from core to the bridge type");
...
/// alef #547: `emit_enum_wrapper` computed `has_cfg_variants` as "any variant has a cfg"
...
/// `_ => unreachable!(...)` catch-all that is dead code once the binding's own feature set
...
fn emit_omits_unreachable_catch_all_for_foreign_variant_proven_unreachable_end_to_end() {
    ...
    let config = swift_config_with_feature(None);
...
fn emit_keeps_catch_all_for_foreign_variant_not_proven_unreachable_end_to_end() {
    let api = foreign_cfg_enum_api();
    let config = swift_config_with_feature(Some("extra-tier"));
    let files = emit(&api, &config).unwrap();
    let lib_rs = lib_rs_content(&files);
    let conversion = core_to_binding_conversion(lib_rs);
