...
pub(super) fn emit_free_function_forwarders(
    ...
    for func in &api.functions {
        ...
        if let Some(capsule_cfg) = capsule::swift_capsule_return_config(func, &capsule_types) {
            if func.is_async {
                capsule::emit_async_capsule_free_function_forwarder(func, &swift_name, capsule_cfg, out);
            } else {
                capsule::emit_capsule_free_function_forwarder(func, &swift_name, capsule_cfg, out);
            }
            continue;
        }
        if let Some(wb_type_name) =
            crate::codegen::mut_writeback::writeback_param(&func.params, &func.return_type, &opaque_types)
                .and_then(crate::codegen::mut_writeback::writeback_type_name)
        {
            writeback::emit_free_function_forwarder(func, &swift_name, wb_type_name, known_dto_names, out);
            continue;
        }
        if func.is_async {
