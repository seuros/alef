...
/// A `frb_generated.dart` already carrying an OLD injected loader (2-step,
/// cache-unaware, with obsolete sibling helpers `_alefHostRid` /
/// `_alefHostLibNames`) — the exact shape that shipped in a released binding
/// before the versioned-cache resolution step was added.
fn stale_injected_fixture() -> &'static str {
    r#"// @generated by `flutter_rust_bridge`@ 2.12.0.
...
class RustLib extends BaseEntrypoint<RustLibApi, RustLibApiImpl, RustLibWire> {
  ...
  static Future<ExternalLibrary?> _alefResolveExternalLibrary() async {
    try {
      ...
      final searchDirs = <Uri>[
        ...
        packageRoot.resolve('src/sample_router_bridge_generated/'),
      ];
  ...
  static List<String> _alefHostLibNames() {
    if (Platform.isMacOS) return const ['sample_router_dart.framework', 'libsample_router_dart.dylib'];
    return const ['libsample_router_dart.so'];
  }
...
#[test]
fn loader_rewrite_upgrades_stale_injected_loader() {
    let out = rewrite_frb_external_library_loader(
        stale_injected_fixture(),
        "sample_router",
        "sample_router",
        "sample_router_dart",
    );
    // The upgraded loader gains the versioned-cache resolution step.
    assert!(
        out.contains("nativeCachedLibPath()"),
        "stale loader must be upgraded to the cache-aware template, got:\n{out}"
    );
    // Obsolete sibling helpers from the old injection are removed.
    assert!(
        !out.contains("_alefHostRid"),
        "obsolete `_alefHostRid` helper must be dropped, got:\n{out}"
    );
    assert!(
        !out.contains("_alefHostLibNames"),
        "obsolete `_alefHostLibNames` helper must be dropped, got:\n{out}"
    );
    // The single resolver helper and the init injection survive exactly once.
    assert_eq!(
        out.matches("_alefResolveExternalLibrary() async").count(),
        1,
        "resolver helper must appear exactly once after upgrade, got:\n{out}"
    );
    assert_eq!(
        out.matches("externalLibrary ??= await _alefResolveExternalLibrary();")
            .count(),
        1,
        "init injection must appear exactly once after upgrade, got:\n{out}"
    );
    // The original `init` body is preserved.
    assert!(
        out.contains("await instance.initImpl("),
        "init body must be preserved after upgrade, got:\n{out}"
    );
    // The native_loader import is added for the cache helpers.
    assert!(
        out.contains("import 'package:sample_router/src/native_loader.dart';"),
        "native_loader import must be present after upgrade, got:\n{out}"
    );
    // The upgrade is now stable: a second pass is a no-op.
    let twice = rewrite_frb_external_library_loader(&out, "sample_router", "sample_router", "sample_router_dart");
    assert_eq!(out, twice, "upgraded loader must be idempotent");
}
