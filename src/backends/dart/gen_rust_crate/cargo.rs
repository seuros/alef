...
mod feature_cfg_tests {
    ...
    /// Regression for alef-task #331: `excluded_default_features` only trimmed the wrapper's
    /// own `[features] default = [...]` array (built from cfg-gated enum variants, see the test
    /// above), never the core dependency's own explicit `features = [...]` line (built from
    /// `[crates.dart].features`). Forwarding an excluded name into that line unions it straight
    /// back into the core crate via Cargo's feature unification, defeating a
    /// `target_dep_overrides` entry that turned it off for a specific cfg target -- the same
    /// defect `RubyConfig::excluded_default_features` fixed for the Magnus crate. Asserts both
    /// directions: the excluded name is dropped from the core dep line, and a name nobody
    /// excluded still is forwarded.
    #[test]
    fn cargo_toml_excludes_named_feature_from_core_dep_line_but_keeps_others() {
        ...
        assert!(
            ...
            "excluded_default_features must drop the name from the core dependency's own \
