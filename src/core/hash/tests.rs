...
/// Two files generated from the same inputs but with different bodies must
/// embed different hashes -- a same-per-repo hash (the observed #524 symptom:
/// two distinct values across an entire consumer repo) means the hash isn't
/// actually covering file content.
#[test]
...
/// The same content hashed twice with the same inputs hash must be stable.
#[test]
