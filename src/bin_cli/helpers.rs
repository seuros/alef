...
/// [`verify_walk`] and [`verify_walk_multi`] so both see the same file set.
fn collect_alef_hashes(base_dir: &std::path::Path) -> Vec<(std::path::PathBuf, String, String)> {
    ...
    while let Some(dir) = stack.pop() {
        ...
        for entry in entries.flatten() {
            ...
            if let Some(disk_hash) = crate::core::hash::extract_hash(&content) {
                found.push((path, disk_hash, content));
            }
        }
    }
    found
...
pub(crate) fn verify_walk(base_dir: &std::path::Path, inputs_hash: &str) -> anyhow::Result<Vec<StaleMismatch>> {
    let mut stale: Vec<StaleMismatch> = collect_alef_hashes(base_dir)
        .into_iter()
        .filter(|(_, disk_hash, content)| crate::core::hash::compute_file_hash(inputs_hash, content) != *disk_hash)
        .map(|(path, disk_hash, content)| StaleMismatch {
            path: path.display().to_string(),
            embedded: disk_hash,
