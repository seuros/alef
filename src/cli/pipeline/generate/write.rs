...
pub struct WriteReport {
    ...
    /// one alef simply does not manage. ~keep
    pub refused_paths: Vec<std::path::PathBuf>,
}
...
pub fn report_refused_writes(report: &WriteReport) {
    ...
    warn!(
        "{} file(s) were NOT written: each already exists, carries no alef provenance marker, and \
         ...
         only be written by writing the file, which is exactly what the guard declines. Review each \
         one, and where alef should own it add a first line reading \
         `<comment-prefix> Generated by alef. Do not edit by hand.`",
        paths.len()
    );
    for path in paths {
...
pub fn write_files_report(files: &[(Language, Vec<GeneratedFile>)], base_dir: &Path) -> anyhow::Result<WriteReport> {
    ...
    let refused_paths = std::sync::Mutex::new(Vec::new());
