...
pub struct AdoptReport {
    ...
    /// Paths adopted through the `.alef/` record because their format cannot carry a
    /// marker at all.
...
/// no marker syntax fall back to the durable `.alef/` record, which is the same proof
/// route the guard already consults for them.
