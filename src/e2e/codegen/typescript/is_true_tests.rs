...
//!
//! Split into its own file rather than added to `typescript/assertions.rs`: that file is
...
/// `Option<DataNode>` presence: before the fix this rendered `expect(result.data).toBe(true)`,
/// which requires strict identity to the literal `true` and is always false for a present
/// non-boolean object -- the test failed at runtime even when `data` was present.
#[test]
fn is_true_on_optional_struct_field_checks_presence() {
    let out = render(&optional_resolver("data"), &is_true_assertion("data"));
    assert_eq!(out, "    expect(result.data ?? null).not.toBeNull();\n");
}
